...
contract Reswap is ERC20, ReentrancyGuard, Ownable {
    ...
    constructor(IERC20[] memory assets, string memory name, string memory symbol) public ERC20(name, symbol) {
        require(bytes(name).length > 0, "Reswap: name is empty");
        require(bytes(symbol).length > 0, "Reswap: symbol is empty");
        require(assets.length == 2, "Reswap: only 2 tokens allowed");

        factory = IFactory(msg.sender);
        tokens = assets;
        for (uint i = 0; i < assets.length; i++) {
            require(!isToken[assets[i]], "Reswap: duplicate tokens");
    ...
    function deposit(uint256[] calldata amounts, uint256[] calldata minAmounts) external payable nonReentrant returns(uint256 fairSupply) {
        ...
        require(amounts.length == _tokens.length, "Reswap: wrong amounts length");
        require(msg.value == (_tokens[0].isBNB() ? amounts[0] : (_tokens[1].isBNB() ? amounts[1] : 0)), "Reswap: wrong value usage");

        uint256[] memory realBalances = new uint256[](amounts.length);
        for (uint i = 0; i < realBalances.length; i++) {
            realBalances[i] = _tokens[i].uniBalanceOf(address(this)).sub(_tokens[i].isBNB() ? msg.value : 0);
        }
        ...
        require(fairSupply > 0, "Reswap: result is not enough");
    ...
    function withdraw(uint256 amount, uint256[] memory minReturns) external nonReentrant {
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ...
            require(i >= minReturns.length || value >= minReturns[i], "Reswap: result is not enough");
    ...
    function swap(IERC20 src, IERC20 dst, uint256 amount, uint256 minReturn, address referral) external payable nonReentrant returns(uint256 result) {
        require(msg.value == (src.isBNB() ? amount : 0), "Reswap: wrong value usage");

        Balances memory balances = Balances({
            src: src.uniBalanceOf(address(this)).sub(src.isBNB() ? msg.value : 0),
        ...
        require(result > 0 && result >= minReturn, "Reswap: return is not enough");
    ...
    function rescueFunds(IERC20 token, uint256 amount) external nonReentrant onlyOwner {
        ...
        for (uint i = 0; i < balances.length; i++) {
            require(tokens[i].uniBalanceOf(address(this)) >= balances[i], "Reswap: access denied");
        }
        require(balanceOf(address(this)) >= BASE_SUPPLY, "Reswap: access denied");
    }
