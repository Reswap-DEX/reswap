...
const Reswap = artifacts.require('ReswapMock');
const Token = artifacts.require('TokenMock');

contract('Reswap', function ([_, wallet1, wallet2]) {
    beforeEach(async function () {
        this.DAI = await Token.new('DAI', 'DAI', 18);
        this.WBNB = await Token.new('WBNB', 'WBNB', 18);
    ...
    describe('Creation', async function () {
        it('should be denied with empty name', async function () {
            await expectRevert(
                Reswap.new([this.WBNB.address, this.DAI.address], '', 'RSWAP'),
                'Reswap: name is empty',
            );
        });

        it('should be denied with empty symbol', async function () {
            await expectRevert(
                Reswap.new([this.WBNB.address, this.DAI.address], 'Reswap', ''),
                'Reswap: symbol is empty',
            );
        });

        it('should be denied with tokens length not eqaul to 2', async function () {
            await expectRevert(
                Reswap.new([], 'Reswap', 'RSWAP'),
                'Reswap: only 2 tokens allowed',
            );

            await expectRevert(
                Reswap.new([this.WBNB.address], 'Reswap', 'RSWAP'),
                'Reswap: only 2 tokens allowed',
            );

            await expectRevert(
                Reswap.new([this.WBNB.address, this.DAI.address, this.USDC.address], 'Reswap', 'RSWAP'),
                'Reswap: only 2 tokens allowed',
            );
        });
        ...
        it('should be denied with token duplicates', async function () {
            await expectRevert(
                Reswap.new([this.DAI.address, this.DAI.address], 'Reswap', 'RSWAP'),
                'Reswap: duplicate tokens',
            );

            await expectRevert(
                Reswap.new([constants.ZERO_ADDRESS, constants.ZERO_ADDRESS], 'Reswap', 'RSWAP'),
                'Reswap: duplicate tokens',
            );
        });

        it('should be allowed for 2 different tokens and non-empty name and symbol', async function () {
            await Reswap.new([this.WBNB.address, this.DAI.address], 'Reswap', 'RSWAP');
        });
    });

    describe('Raw BNB support', async function () {
        beforeEach(async function () {
            this.mooniswap = await Reswap.new([constants.ZERO_ADDRESS, this.DAI.address], 'Reswap', 'RSWAP');
        ...
        it('should support DAI to BNB', async function () {
    ...
    describe('Actions', async function () {
        beforeEach(async function () {
            this.mooniswap = await Reswap.new([this.WBNB.address, this.DAI.address], 'Reswap', 'RSWAP');
            await this.WBNB.mint(wallet1, money.wbnb('1'));
            await this.DAI.mint(wallet1, money.dai('270'));
            await this.WBNB.mint(wallet2, money.wbnb('10'));
            await this.DAI.mint(wallet2, money.dai('2700'));
            await this.WBNB.approve(this.mooniswap.address, money.wbnb('1'), { from: wallet1 });
            await this.DAI.approve(this.mooniswap.address, money.dai('270'), { from: wallet1 });
            await this.WBNB.approve(this.mooniswap.address, money.wbnb('10'), { from: wallet2 });
            await this.DAI.approve(this.mooniswap.address, money.dai('2700'), { from: wallet2 });
        });
        ...
        describe('Initial deposits', async function () {
            it('should be denied with length not equal to 2', async function () {
                await expectRevert(
                    ...
                    'Reswap: wrong amounts length',
                );

                await expectRevert(
                    this.mooniswap.deposit([money.wbnb('1')], [money.zero, money.zero], { from: wallet1 }),
                    'Reswap: wrong amounts length',
                );

                await expectRevert(
                    this.mooniswap.deposit([money.wbnb('1'), money.dai('270'), money.dai('1')], [money.zero, money.zero], { from: wallet1 }),
                    'Reswap: wrong amounts length',
                );
            });
            ...
            it('should be denied for zero amount', async function () {
                await expectRevert(
                    this.mooniswap.deposit([money.wbnb('0'), money.dai('270')], [money.zero, money.zero], { from: wallet1 }),
                    'Reswap: amount is zero',
                );

                await expectRevert(
                    this.mooniswap.deposit([money.wbnb('1'), money.dai('0')], [money.zero, money.zero], { from: wallet1 }),
                    'Reswap: amount is zero',
                );
            });

            it('should check minAmounts on deposit', async function () {
                this.mooniswap.deposit([money.wbnb('1'), money.dai('270')], [money.zero, money.zero], { from: wallet1 });

                await expectRevert(
                    this.mooniswap.deposit([money.wbnb('1'), money.dai('270')], [money.wbnb('1').addn(1), money.dai('270')], { from: wallet2 }),
                    'Reswap: minAmount not reached',
                );

                await expectRevert(
                    this.mooniswap.deposit([money.wbnb('1'), money.dai('270')], [money.wbnb('1'), money.dai('270').addn(1)], { from: wallet2 }),
                    'Reswap: minAmount not reached',
                );
            });

            it('should be allowed with zero minReturn', async function () {
                await this.mooniswap.deposit([money.wbnb('1'), money.dai('270')], [money.zero, money.zero], { from: wallet1 });
                expect(await this.mooniswap.balanceOf(wallet1)).to.be.bignumber.equal(money.dai('270'));
            });
            ...
            it('should give the same shares for the same deposits', async function () {
                ...
                await this.mooniswap.deposit([money.wbnb('1'), money.dai('270')], [money.wbnb('1'), money.dai('270')], { from: wallet2 });
            ...
            it('should give the proportional shares for the proportional deposits', async function () {
                ...
                await this.mooniswap.deposit([money.wbnb('10'), money.dai('2700')], [money.wbnb('10'), money.dai('2700')], { from: wallet2 });
            ...
            it('should give less share on unbalanced deposits', async function () {
                ...
                await this.mooniswap.deposit([money.wbnb('1'), money.dai('271')], [money.wbnb('1'), money.dai('270')], { from: wallet2 });
                expect(await this.mooniswap.balanceOf(wallet2)).to.be.bignumber.equal(money.dai('270').addn(1000));
        ...
        describe('Swaps', async function () {
            ...
            it('should not swap token to itself', async function () {
                const result = await this.mooniswap.getReturn(this.WBNB.address, this.WBNB.address, money.wbnb('1'));
                expect(result).to.be.bignumber.equal(money.zero);

                await expectRevert(
                    this.mooniswap.swap(this.WBNB.address, this.WBNB.address, money.wbnb('1'), money.zero, constants.ZERO_ADDRESS, { from: wallet2 }),
                    'Reswap: return is not enough',
                );
            });

            it('should fail on too small minReturn argument', async function () {
                await expectRevert(
                    this.mooniswap.swap(this.WBNB.address, this.DAI.address, money.wbnb('1'), money.dai('135').addn(1), constants.ZERO_ADDRESS, { from: wallet2 }),
                    'Reswap: return is not enough',
                );
            });

            it('should give 50% of tokenB for 100% of tokenA swap as designed by x*y=k', async function () {
                const wethAdditionBalance = await this.mooniswap.getBalanceForAddition(this.WBNB.address);
                const daiRemovalBalance = await this.mooniswap.getBalanceForRemoval(this.DAI.address);
                const result = await this.mooniswap.getReturn(this.WBNB.address, this.DAI.address, money.wbnb('1'));
                expect(wethAdditionBalance).to.be.bignumber.equal(money.wbnb('1'));
                expect(daiRemovalBalance).to.be.bignumber.equal(money.dai('270'));
                expect(result).to.be.bignumber.equal(money.dai('135'));
                ...
                const received = await trackReceivedToken(
                    this.DAI,
                    wallet2,
                    () => this.mooniswap.swap(this.WBNB.address, this.DAI.address, money.wbnb('1'), money.zero, constants.ZERO_ADDRESS, { from: wallet2 }),
                );
                expect(received).to.be.bignumber.equal(money.dai('135'));
            });
            ...
            it('should be give additive results for the swaps of the same direction', async function () {
                ...
                const wethAdditionBalance1 = await this.mooniswap.getBalanceForAddition(this.WBNB.address);
                const daiRemovalBalance1 = await this.mooniswap.getBalanceForRemoval(this.DAI.address);
                const result1 = await this.mooniswap.getReturn(this.WBNB.address, this.DAI.address, money.wbnb('0.5'));
                expect(wethAdditionBalance1).to.be.bignumber.equal(money.wbnb('1'));
                expect(daiRemovalBalance1).to.be.bignumber.equal(money.dai('270'));
                expect(result1).to.be.bignumber.equal(money.dai('90'));

                // The first swap of 0.5 WBNB to DAI
                const received1 = await trackReceivedToken(
                    this.DAI,
                    wallet2,
                    () => this.mooniswap.swap(this.WBNB.address, this.DAI.address, money.wbnb('0.5'), money.zero, constants.ZERO_ADDRESS, { from: wallet2 }),
                );
                expect(received1).to.be.bignumber.equal(money.dai('90'));

                // Pre-second swap checks
                const wethAdditionBalance2 = await this.mooniswap.getBalanceForAddition(this.WBNB.address);
                const daiRemovalBalance2 = await this.mooniswap.getBalanceForRemoval(this.DAI.address);
                const result2 = await this.mooniswap.getReturn(this.WBNB.address, this.DAI.address, money.wbnb('0.5'));
                expect(wethAdditionBalance2).to.be.bignumber.equal(money.wbnb('1.5'));
                expect(daiRemovalBalance2).to.be.bignumber.equal(money.dai('180'));
                expect(result2).to.be.bignumber.equal(money.dai('45'));

                // The second swap of 0.5 WBNB to DAI
                ...
                // Two 0.5 WBNB swaps are equal to the 1 WBNB swap
            ...
            it('should affect reverse price', async function () {
                ...
                const wethAdditionBalance1 = await this.mooniswap.getBalanceForAddition(this.WBNB.address);
                const daiRemovalBalance1 = await this.mooniswap.getBalanceForRemoval(this.DAI.address);
                const result1 = await this.mooniswap.getReturn(this.WBNB.address, this.DAI.address, money.wbnb('1'));
                expect(wethAdditionBalance1).to.be.bignumber.equal(money.wbnb('1'));
                expect(daiRemovalBalance1).to.be.bignumber.equal(money.dai('270'));
                ...
                // The first swap of 1 WBNB to 135 DAI
                const received1 = await trackReceivedToken(
                    this.DAI,
                    wallet2,
                    () => this.mooniswap.swap(this.WBNB.address, this.DAI.address, money.wbnb('1'), money.zero, constants.ZERO_ADDRESS, { from: wallet2 }),
                ...
                const wethRemovalBalance2 = await this.mooniswap.getBalanceForRemoval(this.WBNB.address);
                const result2 = await this.mooniswap.getReturn(this.DAI.address, this.WBNB.address, money.dai('270'));
                expect(daiAdditionBalance2).to.be.bignumber.equal(money.wbnb('270'));
                expect(wethRemovalBalance2).to.be.bignumber.equal(money.dai('1'));
                expect(result2).to.be.bignumber.equal(money.wbnb('0.5'));
                ...
                const wethRemovalBalance3 = await this.mooniswap.getBalanceForRemoval(this.WBNB.address);
                const result3 = await this.mooniswap.getReturn(this.DAI.address, this.WBNB.address, money.dai('202.5'));
                expect(daiAdditionBalance3).to.be.bignumber.equal(money.dai('202.5'));
                expect(wethRemovalBalance3).to.be.bignumber.equal(money.wbnb('1.5'));
                expect(result3).to.be.bignumber.equal(money.wbnb('0.75'));
                ...
                const wethRemovalBalance4 = await this.mooniswap.getBalanceForRemoval(this.WBNB.address);
                const result4 = await this.mooniswap.getReturn(this.DAI.address, this.WBNB.address, money.dai('135'));
                expect(daiAdditionBalance4).to.be.bignumber.equal(money.dai('135'));
                expect(wethRemovalBalance4).to.be.bignumber.equal(money.wbnb('2'));
                expect(result4).to.be.bignumber.equal(money.wbnb('1'));

                // swap back 135 DAI to 1 BNB
                const received2 = await trackReceivedToken(
                    this.WBNB,
                    wallet2,
                    () => this.mooniswap.swap(this.DAI.address, this.WBNB.address, money.dai('135'), money.zero, constants.ZERO_ADDRESS, { from: wallet2 }),
                );
                expect(received2).to.be.bignumber.equal(money.wbnb('1'));
            });
            ...
            it('should do multiple cross swaps', async function () {
                ...
                await checkBalances(this.mooniswap, this.WBNB, money.wbnb('2'), money.wbnb('2'), money.wbnb('1'));
                ...
                )).to.be.bignumber.equal(money.wbnb('0.5'));

                await checkBalances(this.mooniswap, this.WBNB, money.wbnb('1.5'), money.wbnb('2'), money.wbnb('0.5'));
                await checkBalances(this.mooniswap, this.DAI, money.dai('405'), money.dai('540'), money.dai('135'));

                await timeIncreaseTo(started.add((await this.mooniswap.decayPeriod()).divn(2)));

                await checkBalances(this.mooniswap, this.WBNB, money.wbnb('1.5'), money.wbnb('1.75'), money.wbnb('1'));
                ...
                await checkBalances(this.mooniswap, this.WBNB, money.wbnb('3.25'), money.wbnb('3.5'), money.wbnb('1'));
                ...
                await checkBalances(this.mooniswap, this.WBNB, money.wbnb('2.75'), money.wbnb('3.5'), money.wbnb('0.5'));
        ...
        describe('Deposits after swaps', async function () {
            ...
            it('should deposit imbalanced amount after swap', async function () {
                ...
                await this.mooniswap.swap(this.WBNB.address, this.DAI.address, money.wbnb('1'), money.zero, constants.ZERO_ADDRESS, { from: wallet2 });
            ...
            it('should keep rates after imbalanced deposit', async function () {
                ...
                await checkBalances(this.mooniswap, this.WBNB, money.wbnb('2'), money.wbnb('2'), money.wbnb('1.5'));
                ...
                await checkBalances(this.mooniswap, this.WBNB, money.wbnb('4'), money.wbnb('4'), money.wbnb('3'));
        ...
        describe('Withdrawals', async function () {
            ...
            it('should be able to exit fully', async function () {
                ...
                expect(await this.WBNB.balanceOf(this.mooniswap.address)).to.be.bignumber.equal('4');
            });
            ...
            it('should be able to exit partially', async function () {
                ...
                expect(await this.WBNB.balanceOf(this.mooniswap.address)).to.be.bignumber.equal(money.wbnb('0.5').addn(2));
            ...
            it('2 users should be able to exit', async function () {
                ...
                expect(await this.WBNB.balanceOf(this.mooniswap.address)).to.be.bignumber.equal(money.wbnb('1').addn(4));
        ...
        describe('Rounding', async function () {
            for (const i of ['13', '452', '8000', '14991', '98625']) {
                it('should round virtual balances on withdrawals correctly', async function () {
                    ...
                    const wethBalance = await this.WBNB.balanceOf(this.mooniswap.address);
                    ...
                    expect(await this.mooniswap.getBalanceForAddition(this.WBNB.address)).to.be.bignumber.equal(wethBalance);
                    expect(await this.mooniswap.getBalanceForRemoval(this.WBNB.address)).to.be.bignumber.equal(wethBalance);
                });
            }
        });
